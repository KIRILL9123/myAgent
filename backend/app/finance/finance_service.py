...
def create_recurring_transaction(
    ...
    """Create a recurring template and its first ledger entry atomically."""
    if type not in {"income", "expense"}:
        return {"status": "error", "message": "type must be 'income' or 'expense'"}
    if amount <= 0:
        return {"status": "error", "message": "amount must be greater than zero"}
    try:
        ...
        normalized_currency = normalize_currency(currency, _default_currency())
    except ValueError as exc:
        return {"status": "error", "message": str(exc)}
        ...
        cursor.execute(
            """INSERT INTO recurring_templates
               (type, amount, currency, category, description, day_of_month,
                frequency, active)
               VALUES (?, ?, ?, ?, ?, ?, 'monthly', 1)""",
            (type, amount, normalized_currency, category, description, parsed_date.day),
        )
        template_id = cursor.lastrowid
        cursor.execute(
            """INSERT INTO transactions
               (type, amount, currency, category, description, date,
                recurring_template_id, source_template_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (type, amount, normalized_currency, category, description,
             transaction_date, template_id, template_id),
        )
