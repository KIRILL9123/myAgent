...
export default function FinanceSummaryCards({ summary, formatCurrency, periodLabel }: FinanceSummaryCardsProps) {
  return (
    <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
      <div className="flex min-w-0 items-center justify-between rounded-2xl border border-zinc-900 bg-zinc-900/30 p-4.5">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-base font-bold text-emerald-400 font-mono">{formatCurrency(summary.total_income)}</span>
        </div>
      ...
      <div className="flex min-w-0 items-center justify-between rounded-2xl border border-zinc-900 bg-zinc-900/30 p-4.5">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-base font-bold text-rose-400 font-mono">{formatCurrency(summary.total_expense)}</span>
        </div>
      ...
      <div className="flex min-w-0 items-center justify-between rounded-2xl border border-zinc-900 bg-zinc-900/30 p-4.5">
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Итог за период</span>
          <span className="text-[10px] text-zinc-600">{periodLabel}</span>
          <span
            className={`text-base font-bold font-mono ${summary.net_balance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
          >
