...
import { Calendar, Wallet, Clock, Mail, AlertTriangle, TrendingUp, TrendingDown, ChevronRight } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const calendarQuery = useQuery({
    queryKey: ['dashboard', 'calendar', today],
    queryFn: () => fetchEvents(today, today),
  });
  const financeQuery = useQuery({
    queryKey: ['dashboard', 'finance', today.slice(0, 7)],
    queryFn: () => {
      const now = new Date();
      return fetchSummary(
  ...
  const countdownsQuery = useQuery({ queryKey: ['dashboard', 'countdowns'], queryFn: fetchCountdowns });
  const mailQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return gmail.length + ukrnet.length;
    },
  });

  const calendarEvents = calendarQuery.data ?? [];
  const calCount = calendarEvents.length;
  const firstEvent = calendarEvents.length
    ? [...calendarEvents].sort((a, b) => a.start.localeCompare(b.start))[0]
    : null;
  const financeSum: FinanceSummary | null = financeQuery.data ?? null;
  const urgentCount = (countdownsQuery.data ?? []).filter(
    (countdown) => countdown.days_remaining <= 30 && countdown.days_remaining >= 0,
  ).length;
  const mailCount = mailQuery.data ?? 0;
  const calLoading = calendarQuery.isLoading;
  const finLoading = financeQuery.isLoading;
  const cdLoading = countdownsQuery.isLoading;
  const mailLoading = mailQuery.isLoading;
  const calError = calendarQuery.isError;
  const finError = financeQuery.isError;
  const cdError = countdownsQuery.isError;
  const mailError = mailQuery.isError;
    ...
    <div className="h-full w-full flex flex-col bg-zinc-950 text-zinc-100 overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-4 sm:p-6">
        ...
        <section className="mx-auto mt-8 max-w-5xl" aria-labelledby="dashboard-domains-title">
          <div className="mb-3">
            <h2 id="dashboard-domains-title" className="text-xs font-bold uppercase tracking-widest text-zinc-400">
              Центр управления
            </h2>
          </div>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div
              ...
              className="group bg-zinc-900/30 border border-zinc-900 hover:border-zinc-800/80 rounded-2xl p-5 transition-all cursor-pointer flex flex-col justify-between hover:shadow-lg shadow-md min-h-[170px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/70"
            >
              <div>
                ...
                {calLoading ? (
                  ...
                  <div className="space-y-1.5">
                    <div className="text-xl font-bold font-sans tracking-tight">
                      {calCount === 0
                        ? 'Нет событий сегодня'
                        : `${calCount} ${calCount % 10 === 1 && calCount % 100 !== 11 ? 'событие' : [2, 3, 4].includes(calCount % 10) && ![12, 13, 14].includes(calCount % 100) ? 'события' : 'событий'}`}
                    </div>
                    {firstEvent && (
              ...
              <div className="flex justify-between items-center border-t border-zinc-900/80 pt-4 mt-6">
                ...
                <ChevronRight className="h-4 w-4 text-zinc-600 group-hover:text-zinc-400 transform group-hover:translate-x-1 transition-all" />
              </div>
            </div>
            ...
            <div
              ...
              onKeyDown={(event) => {
                if (event.key === 'Enter' || event.key === ' ') navigate('/finance');
              }}
              className="group bg-zinc-900/30 border border-zinc-900 hover:border-zinc-800/80 rounded-2xl p-5 transition-all cursor-pointer flex flex-col justify-between hover:shadow-lg shadow-md min-h-[170px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/70"
            >
              <div>
                <div className="flex justify-between items-center mb-4">
                ...
                {finLoading ? (
                  ...
                  <div className="space-y-1.5">
                    <div
                      className={`text-xl font-bold font-mono tracking-tight ${financeSum && financeSum.net_balance >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                    >
            ...
            <div
              ...
              role="button"
              tabIndex={0}
              aria-label="Открыть дедлайны"
              onKeyDown={(event) => {
                if (event.key === 'Enter' || event.key === ' ') navigate('/deadlines');
              }}
              className="group bg-zinc-900/30 border border-zinc-900 hover:border-zinc-800/80 rounded-2xl p-5 transition-all cursor-pointer flex flex-col justify-between hover:shadow-lg shadow-md min-h-[170px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400/70"
            >
              <div>
                <div className="flex justify-between items-center mb-4">
                ...
                {cdLoading ? (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="text-xs text-zinc-500 leading-relaxed font-sans">
                      {urgentCount === 0 ? 'Все задачи в плановом режиме' : 'Требуют внимания в ближайшие 30 дней'}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div
              ...
              role="button"
              tabIndex={0}
              aria-label="Открыть почту"
              onKeyDown={(event) => {
                if (event.key === 'Enter' || event.key === ' ') navigate('/mail');
              }}
              className="group bg-zinc-900/30 border border-zinc-900 hover:border-zinc-800/80 rounded-2xl p-5 transition-all cursor-pointer flex flex-col justify-between hover:shadow-lg shadow-md min-h-[170px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/70"
            >
              <div>
                <div className="flex justify-between items-center mb-4">
