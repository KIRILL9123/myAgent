...
Веб-интерфейс доступен по маршруту `/notifications`. Он показывает режимы «Требуют внимания» и «Все сигналы», фильтры по типу, сводные счётчики и переходы в исходные разделы. Доменные действия по-прежнему выполняются только в своих модулях; Notification Center не создаёт вторую копию состояния.
