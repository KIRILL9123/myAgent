...
1. **Проактивный расход по чекам**: При получении почтовых уведомлений с чеками (например, от магазинов или сервисов) агент должен автоматически распознавать чек и в чате предлагать пользователю внести расход в Finance (без необходимости ручного логирования).
2. **Детектор конфликтов в календаре**: На уровне интерфейса CalendarPage.tsx проверять пересечение временных интервалов при планировании встреч и выводить предупреждающую плашку.
3. **Интеграция Календаря с Памятью (Memory)**: Проверять время создаваемого события на соответствие подтвержденным привычкам/предпочтениям (например, "не любит утренние встречи до 10-11 утра") и выводить предупреждение.
4. **Улучшения почтового клиента**: Threading (группировка писем по цепочкам переписки), индикатор непрочитанных на боковой панели навигации.
5. **Автоматическая очистка спама**: Обучить агента анализировать входящие письма в фоновом режиме, классифицировать спам, рекламу и ненужные уведомления (например, алерты от Kleinanzeigen) и самостоятельно перемещать их в корзину/удалять на основе предпочтений пользователя.
6. **Проактивные алерты по финансам**: Отслеживание превышения лимитов в Telegram (например, по категории Еда).
7. **Дедлайны: интерактивные напоминания**: Бот пишет о дедлайнах живым языком (например, напоминание об Ausbildung за 3 дня с вопросом о готовности).
8. **Локальный Speech-to-Text (STT)**: Кнопка записи голоса в веб-чате, использующая whisper.cpp локально на Mac.
9. **Локальный Text-to-Speech (TTS)**: Озвучка ответов ассистента через Kokoro или Piper TTS.
10. **Авто-события из писем**: Фоновый парсер броней/записей в календари.
11. **Умный бюджетный консультант**: Ответы на вопросы вида «Могу я купить монитор за 30 000 руб?» на базе логов Finance.
12. **Интерактивные виджеты**: Drag-and-drop / Gridstack на дашборде.
13. **Эстетика 2.0**: Glassmorphism, View Transitions API, динамические графики.
14. **Интеграция с Home Assistant (низкий приоритет)**: Датчики и базовое управление IoT (отложено, так как мало устройств).
15. **Умный сейф документов (Semantic Document Vault)**: Загрузка и локальное RAG-индексирование PDF (контракты, страховки, договоры). Возможность задавать вопросы по содержанию документов (например, о сроках увольнения/расторжения аренды) и автоматически создавать дедлайны в календаре.
16. **Автономное самосовершенствование (Self-Improving Agent)**: Создание полноценного цикла самокодинга. Агент получает инструменты чтения/записи своих файлов и запуска терминала (сборка, тесты, миграции) в песочнице с циклом самоотладки. Для ускорения разработки использовать лучшие архитектурные решения и код из существующих Open Source решений:
    * **Aider**: Эффективный формат правки кода через блоки «Search/Replace» (Unified Diff) и сжатая карта проекта Repository Map.
    * **OpenHands** (OpenDevin) / **SWE-agent**: Изолированная песочница в Docker для безопасного выполнения тестов и команд сборки.
    * **LangGraph** / **Smolagents**: Фреймворки для создания графов состояний агента, управления памятью и обработки ошибок.

---

## Audit & Misfire Self-Healing Fixes (2026-07-09)

По результатам аудита последнего крупного блока (виджеты, кэширование, подписки) исправлены проблемы с пропуском ночных фоновых задач на MacBook Air во время сна устройства:
1. **Self-Healing подписок**: В `process_recurring_transactions` жесткая проверка дня месяца (`day == day_of_month`) заменена на проверку наступления дня (`day >= target_day`). В сочетании с проверкой на дубликаты это гарантирует, что если компьютер спал в 1:00 ночи, транзакция сработает при первой же утренней загрузке бэкенда в текущем месяце.
2. **Параметр misfire_grace_time**: Для задач планировщика `morning_summary_job`, `nightly_consolidation_job` и `recurring_transactions_job` в `main.py` добавлен параметр `misfire_grace_time=21600` (6 часов). Если Mac спал в момент триггера, планировщик выполнит задачу сразу при пробуждении (в течение 6 часов).
3. **Безопасность консолидации памяти**: Пропуск `nightly_consolidation_job` не критичен функционально. Он не приводит к потере данных, а просто откладывает построение оптимизированных связей в памяти до следующего запуска, что является допустимым по дизайну ограничением.
4. **Ограничение дедупликации подписок**: Два шаблона подписок с идентичными (amount, category, description) в одном месяце заблокируют друг друга. Данное ограничение задокументировано в ROADMAP.md и будет исправлено в будущем через введение `source_template_id` в схему.

---

## Long-Running Resource Audit & Optimization (2026-07-09)
...
## Backend Code Quality Improvements (2026-07-10)
...
3. **Безопасность API и HTTP статус-коды**:
   ...
   - Роутеры `calendar.py` и `mail.py` переписаны с использованием обертки `run_api_tool`, что полностью устранило дублирование кода обработки исключений (DRY).
4. **DRY в Memory Layer**: В `memory_service.py` логика сборки словарей сущности факта из кортежей ответов SQLite выделена во внутренний хелпер `_row_to_fact(row)`. Это позволило устранить дублирование кода в методах `get_pending_facts()`, `get_approved_facts()` и `get_all_facts()`.
5. **Тестирование циклов самовосстановления**: Создан валидационный скрипт `test_json_error.py`, проверяющий поведение оркестратора при передаче невалидных аргументов. Подтверждена работоспособность механизма исправления ошибок синтаксиса JSON (self-repair loop) на базе обновленного формата ошибок.

## Deployment Resilience & Setup Documentation (2026-07-10)

По результатам аудита разворачиваемости системы с нуля (clean-slate deployment) выполнены следующие доработки:
1. **Устранение падения backend при отсутствии frontend/dist**: Ранее бэкенд аварийно завершал работу на этапе импорта, если директория собранных активов SPA (`frontend/dist`) отсутствовала (так как она добавлена в `.gitignore`). Логика в `main.py` изменена на динамическую проверку существования директории. Если папка отсутствует, выводится понятное предупреждение (Warning) в лог, бэкенд запускается штатно, а при обращении к дашборду возвращается JSON-ответ `404 Not Found` с инструкцией по сборке, без падения приложения.
2. **Обновление README.md**:
   - Четко расписаны требования к версии Python 3.11+ и особенности сборки venv на macOS (проблема системного Python 3.9).
   - Описаны неявные системные зависимости: необходимость установки `ffmpeg` для Whisper и скачивания модели `ollama pull qwen2.5:7b`.
   - Внедрена подробная секция о получении API-ключей и App-Specific паролей для iCloud, Gmail, UkrNet, Telegram и локального ключа авторизации.
   - Описан правильный порядок развертывания: сборка фронтенда теперь стоит строго перед запуском бэкенда.
3. **Обновление .env.example**: В шаблон добавлен опциональный параметр `DATABASE_PATH` для переопределения пути к файлу базы данных.
